// ============================================
// Cloud Sync Module
// Mounted under /sync/* by the main worker (src/index.js).
//
// Endpoints:
//   GET  /sync/oauth/start?app=quartet&port=47821
//   GET  /sync/oauth/callback?code=...&state=...
//   GET  /sync/v1/me
//   GET  /sync/v1/state/{app}
//   PUT  /sync/v1/state/{app}    (If-Match header carries etag)
//   DEL  /sync/v1/state/{app}
//
// Storage model in SYNC_KV (separate namespace from OUTER_RIM_KV):
//   oauth_state:{stateToken}  → { app, localPort, createdAt }   (10 min TTL)
//   token:{opaqueToken}       → { githubUserId, githubLogin, app, createdAt }
...
const VALID_APPS = ['outer-rim', 'parallel', 'perimeter', 'quartet'];
...
export async function handleSync(request, env) {
  ...
  if (!env.SYNC_KV) {
    return jsonResponse({ error: 'sync_not_configured', detail: 'SYNC_KV binding missing' }, 500);
  }

  try {
    if (url.pathname === '/sync' || url.pathname === '/sync/') {
      return jsonResponse({ name: 'outer-rim-sync', mounted: '/sync', ok: true });
    }

    if (url.pathname === '/sync/oauth/start') {
      return handleOAuthStart(request, env, url);
    }
    if (url.pathname === '/sync/oauth/callback') {
      return handleOAuthCallback(request, env, url);
    }

    if (url.pathname === '/sync/v1/me') {
      return authed(request, env, (session) =>
        jsonResponse({ githubUserId: session.githubUserId, githubLogin: session.githubLogin })
      );
    }

    const stateMatch = url.pathname.match(/^\/sync\/v1\/state\/([a-z0-9_-]+)$/);
    if (stateMatch) {
...
function isValidApp(app) {
  return VALID_APPS.includes(app);
}
...
async function handleOAuthStart(request, env, url) {
  ...
  const redirectUri = `${url.origin}/sync/oauth/callback`;
...
async function handleOAuthCallback(request, env, url) {
  ...
  const userResp = await fetch('https://api.github.com/user', {
    headers: {
      ...
      'User-Agent': 'micaiahs-worker-sync',
    },
  });
  if (!userResp.ok) {
